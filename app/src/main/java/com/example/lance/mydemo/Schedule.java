...
public class Schedule {
    ...
    public void setNote(String note) {//设置备注
        this.note = note;
    }
    ...
    public void setTimeSet(boolean timeSet) {//设置是否设置具体时间的布尔值
        this.timeSet = timeSet;
        if(!timeSet){
            //如果为false说明没有设置具体时间，则具体时间默认为当天最后一分钟
            time1 = "32:59";
        }
    }

    public boolean isTimeSet() {//得到是否设了时间
        return timeSet;
    }

    public void setAlarmSet(boolean alarmSet) {//设置是否设置闹钟的布尔值
        this.alarmSet = alarmSet;
        if(!timeSet){
            //如果为false说明没有设置闹钟，则闹钟置null
            date2=null;
            time2=null;
        }
    }

    public boolean isAlarmSet() {//得到是否设置了闹钟
        return alarmSet;
    }
    ...
    public static String toDateString(int y,int m,int d){//静态方法，把int型的年月日转换成YYYY/MM/DD
        ...
        sb.append("/");
        sb.append(d<10?"0"+m:""+m);
        return sb.toString();
    }
    public static String toTimeString(int h,int m){//把int型的时分转换成hh:mm
        StringBuffer sb = new StringBuffer();
    ...
    public boolean isPassed(){//让日程设置时间与当前时间相比，判断日程是否已过期
        ...
        String schTime = timeSet?time1:"23:59";//如果日程没有设置时间，则认为过了当天23:59，也就是到了第二天才过期时
    ...
    public String toInsertSql(RcActivity father){//获取schedule对象存入数据库时的sql语句
        ...
        sn = DBUtil.getSNFromPrefs(father);
        sb.append(sn);
        sb.append(",'");
        sb.append(date1);
        sb.append("','");
    ...
    public String toUpdateSql(RcActivity father)//获取schedule对象更新时的sql语句
    {
        ...
        sn=DBUtil.getSNFromPrefs(father);//换成新的sn
        sb.append(sn);
        sb.append(",date1='");
        sb.append(date1);
        sb.append("',time1='");
